package com.example.croptorates.adapter
...
import com.example.croptorates.R
...
class CoinViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    var twentyFourHour = itemView.TwentyFourHour
    var sevenDay = itemView.sevenDay
}
...
class CoinAdapter(recyclerView: RecyclerView, internal var activity: Activity, var items: List<CoinModel>) :
    ...
    override fun onCreateViewHolder(p0: ViewGroup, p1: Int): CoinViewHolder {
        val view = LayoutInflater.from(activity)
            .inflate(R.layout.crypto_layout, p0, false)
    ...
    override fun onBindViewHolder(holder: CoinViewHolder, p1: Int) {
        ...
        item.twentyFourHour.text = coinModel.percent_change_24h + "%"
        item.sevenDay.text = coinModel.percent_change_7d + "%"

        Picasso.get().load(Common.imageURL).into(item.coinIcon)
